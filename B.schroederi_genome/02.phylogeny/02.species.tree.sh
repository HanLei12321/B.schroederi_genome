...
perl orthomcl_pipelinev1.pl --step 123 --input cds.list -outdir output --list category.txt --verbose --mode 3 --type animal &
...
for gene in `less single-copy.gene.list`; do
	...
	Gblocks ${gene}.fasta -b3=8 -b4=5 -b5=h -b6=y;

	# generate single gene tree by RAxML
	raxmlHPC -f a -x 12345 -# 200 -m GTRGAMMA -s ${gene}.fasta-gb -n ${gene}.tre -p 12345;
done
